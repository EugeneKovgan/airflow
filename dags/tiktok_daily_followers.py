...
from common.common_functions import save_parser_history, get_mongo_client
...
def recalculate_tiktok_daily_followers(**kwargs: Dict[str, Any]) -> None:
    ...
    try:
        with MongoClient(os.getenv("MONGO_URL")) as client:
            ...
            previous_stat_date = first_stat['recordCreated']
            if isinstance(previous_stat_date, str):
                previous_stat_date = pendulum.parse(previous_stat_date)
            elif isinstance(previous_stat_date, datetime.datetime):
                previous_stat_date = pendulum.instance(previous_stat_date)
            ...
            for stat in followers_stats[1:]:
                ...
                current_stat_date = stat['recordCreated']
                if isinstance(current_stat_date, str):
                    current_stat_date = pendulum.parse(current_stat_date)
                elif isinstance(current_stat_date, datetime.datetime):
                    current_stat_date = pendulum.instance(current_stat_date)
